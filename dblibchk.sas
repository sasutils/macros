...
populate the macro variables DBTYPE, DBHOST and DBNAME values, respectively.

This will work for both TERADATA and ORACLE libname engines. Not sure
if it works for other external databases.
...
variables that are used by the macro inself.
  ...
  Local macro variables:   DID RC KEEP WHERE LOCALE

Example:
%dblibchk(tdwork)
%put TDWORK is using engine &dbtype on server &dbhost to database &dbname.. ;

----------------------------------------------------------------------*/
%local did rc keep where locale ;
...
-----------------------------------------------------------------------;
  ...
  %if "%sysfunc(getoption(encoding))"="UTF-8" %then %do;
    ...
    ;
    %else %if &locale in (ZH_CN ZH_XX ZH_SG) %then
      %let sysname="E6A8A1E5BC8F2FE794A8E688B7"x
    ;
    %else %if &locale in (ZH_HK ZH_MO ZH_TW) %then
...
observation.
-----------------------------------------------------------------------;
  %let where=sysname=&sysname and libname="&libname";
